...
- Каталог объединяет локализацию, изображения и дополнительные атрибуты в единую структуру `speciesById`. Он также разворачивает данные родов, поэтому каждый вид в файле рода становится самостоятельным вопросом, сохраняя общую сложность и неправильные варианты ответа родительского таксона, если собственные значения не указаны.【F:src/data/catalog.js†L1-L205】
- Файлы родов лежат в `src/data/genus/` и экспортируют структуру с `id`, `slug`, `wrongAnswers` и списком `entries`. Реестр (`index.js`) собирает их в массив и предоставляет поиск по идентификатору, чтобы каталог автоматически подключал все зарегистрированные роды без ручного импорта каждого файла.【F:src/data/genus/index.js†L1-L13】【F:src/data/genus/helianthus.js†L1-L31】
- Для каждого рода используется иерархия идентификаторов: основной уровень задаётся числом (например, `100`), а дочерние виды получают строковые идентификаторы вида `'100_1'`, `'100_2'` и т. д. Фотографии повторяют структуру: изображения рода именуются как `p100_1`, а изображения видов — `p100_1_1`, `p100_2_1` и т. п., что упрощает навигацию и автоматическую привязку снимков.【F:src/data/genus/helianthus.js†L1-L29】【F:src/data/json/plantImages.json†L1-L40】
- При сборке каталога порядок наследования неправильных ответов такой: сначала учитываются индивидуальные `wrongAnswers` конкретного вида (если заданы в `entries`), затем — значения из `speciesCatalog`, и только при их отсутствии берутся базовые `wrongAnswers` самого рода. Верхнеуровневая запись (`100`) тоже получает родовой список автоматически, поэтому его не нужно дублировать внутри `entries`.【F:src/data/catalog.js†L214-L257】【F:src/data/genus/dianthus.js†L1-L29】
- Чтобы настроить отдельную сложность для вида внутри рода, укажите её в `difficulties.js` по ключу вида (`100_1`, `100_2` и т. д.). Если индивидуальной записи нет, игра возьмёт уровень сложности, прописанный для самого рода (`100`), поэтому виды автоматически наследуют настройки таксона верхнего уровня.【F:src/data/catalog.js†L161-L205】【F:src/data/difficulties.js†L1-L25】

## Скрипты
- `npm run lint` — проверка стиля кода и потенциальных ошибок с помощью ESLint.
- `npm run export:data` — собирает каталог растений, изображения, вопросы и параметры в единый `data-export.json`,
  проверяя уникальность идентификаторов и соответствие схеме из `docs/data-json-schema.md`.
...
3. После успешного завершения в корне появится файл `data-export.json`. Скрипт валидирует данные против схемы
   из `docs/data-json-schema.md` и проверяет уникальность идентификаторов вопросов, растений и изображений.
