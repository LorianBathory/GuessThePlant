...
- 📚 В режиме заучивания можно листать все снимки растения из `plantData.json` с помощью стрелок под карточкой — галерея помогает увидеть разные ракурсы.
...
- **PlantData.csv.** В корне репозитория по-прежнему лежит историческая таблица `PlantData.csv`, которую можно использовать для ручного обзора данных. Основной рабочий формат — JSON: актуальные сценарии описаны в `docs/json-data-workflow.md`.【F:PlantData.csv†L1-L20】【F:docs/json-data-workflow.md†L1-L130】
...
- **PlantData.json.** Файл на корневом уровне рассматривается как временный артефакт. Если нужна отдельная копия каталога для внешних инструментов, сформируйте её и проверьте локально перед использованием. Перед коммитом удаляйте или игнорируйте такие файлы, чтобы не засорять историю репозитория.
