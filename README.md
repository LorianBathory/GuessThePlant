...
- Загрузчик [`src/game/dataLoader.js`](src/game/dataLoader.js) объединяет растениеведческие данные из модульных JSON: [`plantData.json`](src/data/json/plantData.json) с игровыми вопросами (включая локализованные названия, ID изображений и списки отвлекающих ответов), [`memorization.json`](src/data/json/memorization.json) с параметрами растений, семействами, родами и списком карточек режима заучивания, а также специализированные режимы — [`bouquetQuestions.json`](src/data/json/bouquetQuestions.json) и другие будущие файлы по аналогии. Все нормализованные наборы складываются в `questionSetsByType`, откуда их читает [`src/data/questions.js`](src/data/questions.js). Благодаря этому структура уже готова к появлению новых категорий вопросов.
- Базовые описания видов и изображений выводятся из записей `plantQuestions` внутри `plantData.json`, а семейства, параметры и доступные роды берутся из `memorization.json`. Структуры сохраняют прежнюю иерархию идентификаторов (`100`, `'100_1'`, `'100_1_1'` и т. п.), поэтому привязка снимков и отвлекающих ответов работает без дублирования данных.
