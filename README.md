...
- Загрузчик [`src/game/dataLoader.js`](src/game/dataLoader.js) объединяет данные из [`src/data/json/plantData.json`](src/data/json/plantData.json), формируя `speciesById`, `plants` и `bouquetQuestions`. Он автоматически подтягивает локализации, изображения и дополнительные атрибуты, чтобы каждый вид превращался в готовый вопрос викторины без ручного связывания модулей.
- Базовые описания родов и видов редактируются в JSON-файлах [`genus.json`](src/data/json/genus.json) и [`speciesCatalog.json`](src/data/json/speciesCatalog.json). Идентификаторы по-прежнему используют иерархию `100`, `'100_1'`, `'100_1_1'`, что облегчает поиск связанных снимков в [`plantImages.json`](src/data/json/plantImages.json).
- Наследование `wrongAnswers` и изображений выполняется в `buildSpeciesData` внутри загрузчика: сначала учитываются поля конкретного вида, затем переопределения из `speciesCatalog`, и только после этого — общие настройки рода. Это гарантирует, что верхнеуровневые записи (`100`) автоматически получают родовые варианты без дублирования данных.
- Уровни сложности и связи «вопрос ↔ сложность» теперь живут в [`difficulties.json`](src/data/json/difficulties.json). `dataLoader` собирает их в `difficultyLevels`, `questionIdsByDifficulty` и `imageIdsByDifficulty`, которыми пользуются `gameConfig` и аналитические скрипты.
