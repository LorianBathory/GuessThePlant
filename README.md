...
- Загрузчик [`src/game/dataLoader.js`](src/game/dataLoader.js) объединяет данные из [`src/data/json/plantData.json`](src/data/json/plantData.json), формируя `speciesById`, `plants` и `bouquetQuestions`. Он автоматически подтягивает локализации, изображения и дополнительные атрибуты, чтобы каждый вид превращался в готовый вопрос викторины без ручного связывания модулей. Исключение — файл [`src/data/json/bouquetQuestions.json`](src/data/json/bouquetQuestions.json), который сохраняется как исходник для букетных заданий и используется экспортерами, потому что автоматический конвертер не умеет работать с этой категорией вопросов.
- Базовые описания родов, видов и изображений находятся напрямую в [`plantData.json`](src/data/json/plantData.json): разделы `genus`, `species` и `plantImages` используют прежнюю иерархию идентификаторов (`100`, `'100_1'`, `'100_1_1'` и т. п.), поэтому привязка снимков и отвлекающих ответов сохраняется без дублирования данных.
...
- Уровни сложности и связи «вопрос ↔ сложность» описаны в разделе `difficulties` внутри `plantData.json`. `dataLoader` собирает их в `difficultyLevels`, `questionIdsByDifficulty` и `imageIdsByDifficulty`, которыми пользуются `gameConfig` и аналитические скрипты.

## Скрипты
- `npm run lint` — проверка стиля кода и потенциальных ошибок с помощью ESLint.
- `npm run export:data` — пересобирает `src/data/json/plantData.json`, сортируя записи и подтягивая параметры из `src/data/plantParameters.js`.
...
3. После успешного завершения будет перезаписан файл `src/data/json/plantData.json`. Скрипт сортирует записи и дополняет их параметрами из `src/data/plantParameters.js`, после чего данные можно проверить `npm run validate:data`.
