...
- **`plantQuestions`** — единственный источник правды находится в `src/data/json/plantData.json` (раздел `plantQuestions`). Любые попытки продублировать или перенести вопросы о растениях в другие файлы запрещены.
