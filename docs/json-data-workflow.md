...
Каталог `src/data/json/plantData.json` считается каноническим источником данных. После ручных правок запускайте `npm run export:data`, чтобы пересобрать легаси-бандл `docs/legacy/plantData.bundle.json`, и `npm run validate:data`, чтобы проверить его на соответствие схеме. Эти команды помогают заметить несоответствия в путях к изображениям, уровнях сложности и перекрёстных ссылках до того, как изменения попадут в игру.
