export const plantNamesById = Object.freeze({
  1:  { ru: "Астерискус приморский", en: "Beach Daisy", nl: "Zeegoudsbloem", sci: "Asteriscus maritimus" },
  2:  { ru: "Дикая морковь", en: "Sea Carrot", nl: "Peen, Wilde peen", sci: "Daucus carota" },
  3:  { ru: "Агапантус", en: "Agapanthus", nl: "Afrikaanse lelie", sci: "Agapanthus africanus" },
  4:  { ru: "Бугенвиллия", en: "Bougainvillea", nl: "Bouganville", sci: "Bougainvillea spectabilis" },
  5:  { ru: "Камелия", en: "Camellia", nl: "Japanse roos", sci: "Camellia japonica" },
  6:  { ru: "Гербера", en: "Gerbera", nl: "Gerbera", sci: "Gerbera jamesonii" },
  7:  { ru: "Арктотека ноготковая", en: "Cape Weed", nl: "Kaapse goudsbloem", sci: "Arctotheca calendula" },
  8:  { ru: "Гацания", en: "Gazania", nl: "Gazania, Middagbloem", sci: "Gazania" },
  9:  { ru: "Лампрантус золотой", en: "Golden Ice Plant", nl: "Gele ijsbloem", sci: "Lampranthus aureus" },
  10: { ru: "Борщевик", en: "Hogweed", nl: "Berenklauw", sci: "Heracleum" },
  11: { ru: "Дудник лесной", en: "Wild Parsley", nl: "Gewone engelwortel", sci: "Angelica sylvestris" },
  12: { ru: "Пастернак", en: "Parsnip", nl: "Pastinaak", sci: "Pastinaca sativa" },
  //13: allium
  14: { ru: "Камассия", en: "Camas", nl: "Prairielelie", sci: "Camassia" },
  15: { ru: "Глобулярия", en: "Globe Daisy", nl: "Kogelbloem", sci: "Globularia" },
  16: { ru: "Рябчик", en: "Fritillary", nl: "Keizerskroon", sci: "Fritillaria" },
  17: { ru: "Клематис", en: "Clematis", nl: "Bosrank", sci: "Clematis" },
  18: { ru: "Глициния", en: "Wisteria", nl: "Blauwe regen", sci: "Wisteria" },
  19: { ru: "Олеандр", en: "Oleander", nl: "Oleander", sci: "Nerium oleander" },
  20: { ru: "Рябина", en: "Mountain-ash", nl: "Lijsterbes", sci: "Sorbus" },
  21: { ru: "Рододендрон", en: "Rhododendron", nl: "Rododendron, Alpenroos", sci: "Rhododendron" },
  22: { ru: "Тюльпан", en: "Tulip", nl: "Tulp", sci: "Tulipa" },
  23: { ru: "Гардения", en: "Gardenia", nl: "Gardenia", sci: "Gardenia" },
  24: { ru: "Магнолия", en: "Magnolia", nl: "Magnolia, Beverboom", sci: "Magnolia" },
  25: { ru: "Нивяник обыкновенный", en: "Oxeye daisy", nl: "Gewone margriet", sci: "Leucanthemum vulgare" },
  26: { ru: "Георгина", en: "Dahlia", nl: "Dahlia", sci: "Dahlia" },
  27: { ru: "Эхинацея", en: "Coneflower", nl: "Zonnehoed", sci: "Echinacea" },
  28: { ru: "Гомфрена", en: "Gomphrena / Globe amaranth", nl: "Kogelamarant", sci: "Gomphrena" },
  29: { ru: "Хризантема", en: "Chrysanthemum", nl: "Chrysant", sci: "Chrysanthemum" },
  30: { ru: "Остеоспермум", en: "African Daisy", nl: "Kaaps ganzenbloempje", sci: "Osteospermum" },
  //31: rosa
  //'31_1': rosa
  32: { ru: "Гузмания", en: "Scarlet Star", nl: "Guzmania", sci: "Guzmania lingulata" },
  33: { ru: "Мак", en: "Poppy", nl: "Klaproos, Papaver", sci: "Papaver" },
  34: { ru: "Похутукава", en: "Pohutukawa", nl: "Nieuw-Zeelandse kerstboom", sci: "Metrosideros excelsa" },
  35: { ru: "Лилия", en: "Lily", nl: "Lelie", sci: "Lilium" },
  //36: hemerocallis
  37: { ru: "Амариллис", en: "Amaryllis", nl: "Amaryllis", sci: "Amaryllis" },
  38: { ru: "Ромашка", en: "Mayweed", nl: "Reukeloze kamille", sci: "Tripleurospermum" },
  39: { ru: "Гелиопсис", en: "False Sunflower", nl: "Zonneoog", sci: "Heliopsis helianthoides" },
  40: { ru: "Кореопсис", en: "Tickseed", nl: "Meisjesogen", sci: "Coreopsis" },
  //genusId: 41 dianthus
  // заменить 42: { ru: "Флокс шиловидный", en: "Moss phlox", nl: "Kruipende vlambloem", sci: "Phlox subulata" },
  43: { ru: "Сансевиерия", en: "Sansevieria", nl: "Vrouwentong", sci: "Sansevieria" },
  44: { ru: "Ананас", en: "Pineapple", nl: "Ananas", sci: "Ananas comosus" },
  45: { ru: "Юкка", en: "Yucca", nl: "Yucca, Palmboom", sci: "Yucca" },
  46: { ru: "Пион", en: "Peony", nl: "Pioen", sci: "Paeonia" },
  //{ genusId: 47 }, anemone
  48: { ru: "Каллистемон", en: "Bottlebrushes", nl: "Schoonsteenveger", sci: "Callistemon" },
  49: { ru: "Мирт", en: "Myrtle", nl: "Mirte", sci: "Myrtus" },
  50: { ru: "Жакаранда", en: "Blue jacaranda", nl: "Jacaranda", sci: "Jacaranda mimosifolia" },
  51: { ru: "Гранат", en: "Pomegranate", nl: "Granaatappel", sci: "Punica granatum" },
  52: { ru: "Смолосемянник", en: "Japanese pittosporum", nl: "Pittosporum", sci: "Pittosporum tobira" },
  53: { ru: "Скабиоза", en: "Sweet scabious", nl: "Weduwebloem", sci: "Sixalix atropurpurea" },
  54: { ru: "Лантана", en: "Lantana", nl: "Lantana", sci: "Lantana" },
  55: { ru: "Гибискус", en: "Hibiscus", nl: "Hibiscus, Roos van Sharon", sci: "Hibiscus" },
  56: { ru: "Робиния (лжеакация)", en: "Black locust", nl: "Witte (valse) acacia", sci: "Robinia pseudoacacia" },
  57: { ru: "Павловния", en: "Princess Tree", nl: "Paulownia, Keizersboom", sci: "Paulownia" },
  58: {ru: "Плющ", en: "Common ivy", nl: "Klimop", sci: "Hedera Helix"},
  59: { ru: "Боярышник", en: "Hawthorn", nl: "Meidoorn", sci: "Crataegus" },
  60: { ru: "Драгонфрут", en: "Dragon Fruit", nl: "Pitahaya, Drakenfruit", sci: "Hylocereus" },
  61: { ru: "Шеффлера", en: "Schefflera", nl: "Vingersboom", sci: "Schefflera" },
  62: { ru: "Яблоня", en: "Apple", nl: "Appel, Appelboom", sci: "Malus" },
  63: { ru: "Кукуруза", en: "Corn", nl: "Maïs", sci: "Zea mays" },
  64: { ru: "Вербена", en: "Verbena", nl: "Ijzerhard", sci: "Verbena" },
  65: { ru: "Незабудка", en: "Forget-me-not", nl: "Vergeet-mij-nietje", sci: "Myosotis" },
  66: { ru: "Мята", en: "Mint", nl: "Munt", sci: "Mentha" },
  67: { ru: "Лаватера", en: "Tree mallow", nl: "Bekermalva", sci: "Lavatera" },
  68: { ru: "Лаванда", en: "Lavender", nl: "Lavendel", sci: "Lavandula" },
  69: { ru: "Эустома", en: "Lisianthus", nl: "Lisianthus, Prairie-gentiaan", sci: "Eustoma" },
  70: { ru: "Физалис", en: "Groundcherry", nl: "Lampionplant", sci: "Physalis" },
  71: { ru: "Каштан", en: "Chestnut", nl: "Kastanje", sci: "Castanea" },
  72: { ru: "Лопух", en: "Burdock", nl: "Klis", sci: "Arctium" },
  73: { ru: "Астра", en: "Aster", nl: "Aster", sci: "Aster" },
  74: { ru: "Флокс", en: "Phlox", nl: "Vlambloem", sci: "Phlox" },
  75: { ru: "Сирень", en: "Lilac", nl: "Sering", sci: "Syringa" },
  76: { ru: "Петуния", en: "Petunia", nl: "Petunia", sci: "Petunia" },
  77: { ru: "Родомирт", en: "Rose Myrtle", nl: "Rhodomyrtus", sci: "Rhodomyrtus" },
  78: { ru: "Картофельное дерево", en: "Potato Tree", nl: "Nachtschade", sci: "Lycianthes (Solanum) rantonnetii" },
  79: { ru: "Свинчатка", en: "Leadwort", nl: "Loodkruid", sci: "Plumbago" },
  80: { ru: "Каладиум", en: "Caladium", nl: "Caladium, Olifantsoor", sci: "Caladium bicolor" },
  81: { ru: "Хоста", en: "Hosta", nl: "Hartlelie, Funkia", sci: "Hosta" },
  82: { ru: "Анютины глазки", en: "Pansy", nl: "Viooltje, Driekleurig viooltje", sci: "Viola tricolor var. hortensis" }, //
  //83: campanula
  84: { ru: "Кольраби", en: "Kohlrabi", nl: "Koolrabi", sci: "Brassica oleracea Gongylodes Group" },
  85: { ru: "Будлея", en: "Butterfly bush", nl: "Vlinderstruik", sci: "Buddleja davidii" },
  86: { ru: "Плюмерия", en: "Frangipani", nl: "Frangipani", sci: "Plumeria" },
  87: { ru: "Лотос", en: "Lotus", nl: "Lotus", sci: "Nelumbo nucifera" },
  88: { ru: "Гортензия", en: "Hydrangea", nl: "Hortensia", sci: "Hydrangea macrophylla" },
  89: { ru: "Сантолина", en: "Cotton lavender", nl: "Heiligenbloem", sci: "Santolina chamaecyparissus" },
  90: { ru: "Бук", en: "Beech", nl: "Beuk", sci: "Fagus sylvatica" },
  91: { ru: "Бегония", en: "Begonia", nl: "Begonia", sci: "Begonia" },
  92: { ru: "Ранункулюс", en: "Ranunculus", nl: "Ranonkel", sci: "Ranunculus" },
  93: { ru: "Сингониум", en: "Arrowhead vine", nl: "Syngonium", sci: "Syngonium podophyllum" },
  94: { ru: "Орхидея", en: "Orchid", nl: "Orchidee", sci: "Orchidaceae" },
  95: { ru: "Банан", en: "Banana", nl: "Banaan", sci: "Musa" },
  96: { ru: "Замиокулькас", en: "ZZ plant", nl: "Vetplant, ZZ-plant", sci: "Zamioculcas zamiifolia" },
  97: { ru: "Бархатцы", en: "Marigold", nl: "Afrikaantje", sci: "Tagetes" },
  98: { ru: "Маракуйя", en: "Passion fruit", nl: "Passievrucht, Passiebloemfruit", sci: "Passiflora edulis" },
  99: { ru: "Циссус", en: "Grape ivy", nl: "Klimop", sci: "Cissus rhombifolia" },
  //100: helianthus
  101: { ru: "Космея", en: "Cosmos", nl: "Cosmea", sci: "Cosmos bipinnatus" },
  102: { ru: "Аглаонема", en: "Aglaonema / Chinese evergreen", nl: "Aglaonema", sci: "Aglaonema" },
  103: { ru: "Примула", en: "Primrose", nl: "Sleutelbloem", sci: "Primula" },
  104: { ru: "Сцилла", en: "Scilla / Squill", nl: "Sterhyacint", sci: "Scilla" },
  105: { ru: "Нарцисс", en: "Daffodil", nl: "Narcis", sci: "Narcissus" },
  //106: lupinus
  //'106_1': lupinus
  107: { ru: "Мускари", en: "Grape hyacinth", nl: "Blauwe druifjes", sci: "Muscari" },
  108: { ru: "Пеларгония (Герань)", en: "Geranium (Pelargonium)", nl: "Geranium / Pelargonium", sci: "Pelargonium" },
  109: { ru: "Калибрахоа", en: "Calibrachoa / Million bells", nl: "Calibrachoa", sci: "Calibrachoa" },
  110: { ru: "Колеус", en: "Coleus", nl: "Siernetel", sci: "Coleus" },
  //'83_1': campanula
  112: { ru: "Дельфиниум", en: "Delphinium", nl: "Ridderspoor", sci: "Delphinium" },
  113: { ru: "Брунера", en: "Brunnera / Siberian bugloss", nl: "Kaukasisch vergeet-mij-nietje", sci: "Brunnera" },
  114: { ru: "Береза", en: "Birch", nl: "Berk", sci: "Betula" },
  115: { ru: "Рудбекия", en: "Rudbeckia / Black-eyed Susan", nl: "Zonnehoed", sci: "Rudbeckia" },
  //116: centaurea
  //'116_1': centaurea
  118: { ru: "Журавельник (Герань)", en: "Hardy Geranium / Crane's-bill", nl: "Ooievaarsbek", sci: "Geranium" },
  119: { ru: 'Маргаритка', en: 'English daisy', nl: 'Tuinmadeliefje', sci: 'Bellis perennis' },
  120: { ru: "Дицентра", en: "Bleeding heart", nl: "Gebroken hartje", sci: "Lamprocapnos (old: Dicentra)" },
  121: { ru: "Гайлардия", en: "Gaillardia / Blanket flower", nl: "Kokardebloem", sci: "Gaillardia" },
  122: { ru: "Галантус", en: "Snowdrop", nl: "Sneeuwklokje", sci: "Galanthus" },
  123: { ru: "Целозия", en: "Celosia / Cockscomb", nl: "Hanekam", sci: "Celosia" },
  124: { ru: "Опунция", en: "Opuntia / Prickly pear cactus", nl: "Schijfcactus", sci: "Opuntia" },
  125: { ru: "Ширококолокольчик", en: "Platycodon / Balloon flower", nl: "Ballonklokje", sci: "Platycodon grandiflorus" },
  126: { ru: "Эвкалипт", en: "Eucalyptus", nl: "Eucalyptus", sci: "Eucalyptus" },
  127: { ru: "Цинния", en: "Zinnia", nl: "Zinnia", sci: "Zinnia" },
  128: { ru: "Карпобротус съедобный", en: "Pigface / Hottentot fig", nl: "Carpobrotus", sci: "Carpobrotus edulis" },
  //'13_1': allium
  130: { ru: "Крокус", en: "Crocus", nl: "Krokus", sci: "Crocus" },
  //131: veronica
  132: { ru: "Фризея", en: "Vriesea", nl: "Vriesea", sci: "Vriesea" },
  133: { ru: "Неорегелия", en: "Neoregelia", nl: "Neoregelia", sci: "Neoregelia" },
  134: { ru: "Эхмея", en: "Aechmea", nl: "Aechmea", sci: "Aechmea" },
  136: { ru: "Банксия", en: "Banksia", nl: "Banksia", sci: "Banksia" },
  137: { ru: "Бобовник", en: "Golden chain", nl: "Goudenregen", sci: "Laburnum" },
  138: { ru: "Львиный зев", en: "Snapdragon", nl: "Vlambloem / Leeuwenbek", sci: "Antirrhinum" },
  139: { ru: "Звездчатый жасмин", en: "Star jasmine", nl: "Sterjasmijn", sci: "Trachelospermum jasminoides" },
  140: { ru: "Жимолость татарская", en: "Tatarian honeysuckle", nl: "Tartaarse kamperfoelie", sci: "Lonicera tatarica" },
  141: { ru: "Софора японская", en: "Japanese pagoda tree", nl: "Honingboom", sci: "Sophora japonica" },
  142: { ru: "Гиацинт", en: "Hyacinth", nl: "Hyacint", sci: "Hyacinthus" },
  144: { ru: "Астильба", en: "Astilbe", nl: "Astilbe", sci: "Astilbe" },
  145: { ru: "Агростемма", en: "Corncockle", nl: "Bolderik", sci: "Agrostemma" },
  146: { ru: "Клевер", en: "Red Clover", nl: "Rode klaver", sci: "Trifolium pratense" },
  147: { ru: "Молодило", en: "Houseleek", nl: "Huislook", sci: "Sempervivum" },
  148: { ru: "Фатсия", en: "Fatsia", nl: "Fatsia", sci: "Fatsia" },
  149: { ru: "Пахира", en: "Pachira", nl: "Pachira", sci: "Pachira" },
  //150: salvia
  //'150_1': salvia
  151: { ru: "Плектрантус", en: "Plectranthus", nl: "Plectranthus", sci: "Plectranthus" },
  152: { ru: "Мальва", en: "Common mallow", nl: "Kaasjeskruid", sci: "Malva" },
  153: { ru: "Кизил", en: "Flowering Dogwood", nl: "Oostamerikaanse kornoelje", sci: "Cornus Florida" },
  154: { ru: "Эджевортия", en: "Edgeworthia", nl: "Edgeworthia", sci: "Edgeworthia chrysantha" },
  155: { ru: "Ангелония", en: "Angelonia", nl: "Angelonia", sci: "Angelonia" },
  156: { ru: "Артишок", en: "Artichoke", nl: "Artisjok", sci: "Cynara cardunculus" },
  157: { ru: "Жасмин многоцветковый", en: "Jasmine", nl: "Jasmijn", sci: "Jasminum polyanthum" },
  158: { ru: "Санчезия", en: "Sanchezia", nl: "Sanchezia", sci: "Sanchezia" },
  159: { ru: "Агератум", en: "Ageratum", nl: "Ageratum", sci: "Ageratum" },
  160: { ru: "Аквилегия", en: "Columbine", nl: "Akelei", sci: "Aquilegia vulgaris" },
  161: { ru: "Обриета", en: "Aubrieta", nl: "Blauwkussen", sci: "Aubrieta" },
  162: { ru: "Катарантус", en: "Catharanthus", nl: "Catharanthus", sci: "Catharanthus" },
  163: { ru: "Фитония", en: "Fittonia", nl: "Fittonia", sci: "Fittonia" },
  164: { ru: "Дымянка", en: "Fumitory", nl: "Duivenkervel", sci: "Fumaria" },
  165: { ru: "Морозник", en: "Hellebore", nl: "Nieskruid", sci: "Helleborus" },
  166: { ru: "Шток-роза", en: "Hollyhock", nl: "Stokroos", sci: "Alcea" },
  167: { ru: "Цикорий", en: "Chicory", nl: "Wilde Cichorei", sci: "Cichorium Intybus" },
  168: { ru: "Кувшинка", en: "Water Lily", nl: "Waterlelie", sci: "Nymphaea" },
  //169: amaranthus
  //'169_1': amaranthus
  //'169_2': amaranthus
  171: { ru: "Цеанотус (краснокоренник) вдавленный", en: "Santa Barbara ceanothus", nl: "Amerikaanse sering", sci: "Ceanothus Impressus" },
  172: { ru: "Бересклет", en: "Spindle tree", nl: "Kardinaalsmuts", sci: "Euonymus" },
  //173: hebe
  174: { ru: "Алоэ складчатое", en: "Fan aloe", nl: "Waaier-aloë", sci: "Kumara Plicatilis" },
  175: { ru: "Левизия котиледон (туполистная)", en: "Siskiyou lewisia", nl: "Lewisia", sci: "Lewisia Cotyledon" },
  176: { ru: "Павловния", en: "Paulownia / Princess tree", nl: "Anna Paulownaboom", sci: "Paulownia" },
  177: { ru: "Фацелия", en: "Phacelia / Scorpionweed", nl: "Phacelia", sci: "Phacelia" },
  178: { ru: "Лавровишня", en: "Cherry laurel", nl: "Laurierkers", sci: "Prunus Laurocerasus" },
  179: { ru: "Пираканта алая", en: "Firethorn / Scarlet firethorn", nl: "Vuurdoorn", sci: "Pyracantha Coccinea" },
  181: { ru: "Одуванчик", en: "Dandelion", nl: "Paardenbloem", sci: "Taraxacum Officinale" },
  //'47_1': anemone
  183: { ru: "Антуриум", en: "Anthurium / Flamingo flower", nl: "Flamingoplant", sci: "Anturium" },
  184: { ru: "Вереск", en: "Heather", nl: "Struikheide", sci: "Calluna" },
  185: { ru: "Эшшольция калифорнийская", en: "California poppy", nl: "Californische klaproos", sci: "Eschscholzia Californica" },
  186: { ru: "Гелениум осенний", en: "Sneezeweed", nl: "Zonnekruid", sci: "Helenium Autumnale" },
  187: { ru: "Геликония попугайная", en: "Parrot’s flower / Parrot heliconia", nl: "Papegaaienbloem", sci: "Heliconia Psittacorum" },
  188: { ru: "Каланхоэ Блоссфельда", en: "Flaming Katy / Kalanchoe", nl: "Flamingoplantje / Kalanchoë", sci: "Kalanchoe Blossfeldiana" },
  189: { ru: "Земляника садовая", en: "Strawberry", nl: "Aardbei", sci: "Fragaria × ananassa" },
  190: { ru: "Декабрист", en: "Christmas cactus", nl: "Kerstcactus", sci: "Schlumbergera Truncata" },
  191: { ru: "Ландыш майский", en: "Lily of the valley", nl: "Lelietje-van-dalen", sci: "Convallaria Majalis" },
  //'41_1': dianthus
  193: { ru: "Смолёвка халцедоновая", en: "Maltese cross", nl: "Brandende liefde", sci: "Silene chalcedonica" },
  194: { ru: "Гелихризум сухоцвет", en: "Strawflower / Golden everlasting", nl: "Strobloem", sci: "Xerochrysum bracteatum" },
  195: { ru: "Львиный зев", en: "Snapdragon", nl: "Leeuwebek", sci: "Antirrhinum majus" },
  196: { ru: "Юстиция розовая", en: "Brazilian plume flower", nl: "Braziliaanse pluim", sci: "Justicia carnea" },
  197: { ru: "Аихризон", en: "Aichryson / Houseleek tree", nl: "Vetplantje / Aichryson", sci: "Aichryson" },
  //'150_2': salvia
  //'106_2': lupinus
  200: { ru: "Кориандр (кинза)", en: "Coriander / Cilantro", nl: "Koriander", sci: "Coriandrum sativum" },
  201: { ru: "Барвинок малый", en: "Lesser periwinkle", nl: "Kleine maagdenpalm", sci: "Vinca Minor" },
  202: { ru: "Прострел", en: "Pasque flower", nl: "Wildemanskruid", sci: "Pulsatilla" },
  203: { ru: "Катальпа", en: "Catalpa", nl: "Trompetboom", sci: "Catalpa" },
  204: { ru: "Маниок", en: "Cassava", nl: "Cassave", sci: "Cassava" },
  205: { ru: "Очиток", en: "Stonecrop (Sedum)", nl: "Vetkruid (Sedum)", sci: "Sedum" },
  206: { ru: "Спатифиллум", en: "Peace lily", nl: "Lepelplant", sci: "Spathiphyllum" },
  207: { ru: "Алиссум (бурачок)", en: "Alyssum (Sweet alyssum)", nl: "Schildzaad", sci: "Alyssum" },
  208: { ru: "Гелиантемум (солнечник)", en: "Sunrose (rock rose)", nl: "Zonneroosje", sci: "Helianthemum" },
  209: { ru: "Стробилантес", en: "Strobilanthes (Persian shield)", nl: "Strobilanthes", sci: "Strobilanthes" },
  210: { ru: "Валериана", en: "Valerian", nl: "Valeriaan", sci: "Valeriana" },
  211: { ru: "Калина", en: "Viburnum", nl: "Sneeuwbal", sci: "Viburnum" },
  212: { ru: "Виноград", en: "Grape vine", nl: "Wijnstok (Druif)", sci: "Vitis vinifera" },
  213: { ru: "Уитфилдия", en: "Whitfieldia (Whitefieldia)", nl: "Whitfieldia", sci: "Whitfieldia" },
  214: { ru: "Укроп", en: "Dill", nl: "Dille", sci: "Anethum graveolens" },
  215: { ru: "Баухиния", en: "Orchid tree", nl: "Orchideeënbloem (Bauhinia)", sci: "Bauhinia" },
  216: { ru: "Барбарис", en: "Barberry", nl: "Berberis", sci: "Berberis" },
  217: { ru: "Крокосмия (монбреция)", en: "Montbretia (Crocosmia)", nl: "Montbretia", sci: "Crocosmia" },
  218: { ru: "Тыква", en: "Pumpkin", nl: "Pompoen", sci: "Cucurbita pepo" },
  219: { ru: "Цикламен", en: "Cyclamen", nl: "Alpenviooltje", sci: "Cyclamen" },
  220: { ru: "Дуранта", en: "Sky flower (Golden dewdrop)", nl: "Duranta", sci: "Duranta repens" },
  221: { ru: "Эрантис (весенник)", en: "Winter aconite", nl: "Winterakoniet", sci: "Eranthis" },
  222: { ru: "Чистяк (лютик) весенний", en: "Lesser celandine", nl: "Speenkruid", sci: "Ficaria" },
  223: { ru: "Фуксия", en: "Fuchsia", nl: "Fuchsia", sci: "Fuchsia" },
  224: { ru: "Плющ обыкновенный", en: "English ivy", nl: "Klimop", sci: "Hedera helix" },
  225: { ru: "Ипомея", en: "Morning glory", nl: "Winde (Ipomoea)", sci: "Ipomoea" },
  226: { ru: "Ятрофа", en: "Peregrina", nl: "Jatropha", sci: "Jatropha integerrima" },
  227: { ru: "Копсия", en: "Kopsia", nl: "Kopsia", sci: "Kopsia" },
  228: { ru: "Лавр благородный", en: "Bay laurel", nl: "Laurier", sci: "Laurus nobilis" },
  229: { ru: "Бадьян (анисовое дерево)", en: "Japanese star anise", nl: "Japanse steranijs", sci: "Illicium anisatum" },
  230: { ru: "Ликорис лучистый", en: "Red spider lily", nl: "Rode spinlelie", sci: "Lycoris radiata" },
  231: { ru: "Монарда двойчатая", en: "Bee balm (Bergamot)", nl: "Bergamotplant", sci: "Monarda didyma" },
  232: { ru: "Кубышка жёлтая", en: "Yellow water-lily", nl: "Gele plomp", sci: "Nuphar lutea" },
  233: { ru: "Пахистахис", en: "Golden shrimp plant", nl: "Gele kegelplant", sci: "Pachystachys" },
  234: { ru: "Остролист (падуб)", en: "Holly", nl: "Hulst", sci: "Ilex aquifolium" },
  235: { ru: "Кислица треугольная", en: "False shamrock", nl: "Driehoekklaver", sci: "Oxalis triangularis" },
  236: { ru: "Девичий виноград пятилисточковый", en: "Virginia creeper", nl: "Wilde wingerd", sci: "Parthenocissus quinquefolia" },
  237: { ru: "Петрея", en: "Queen’s wreath", nl: "Kransbloem", sci: "Petrea volubilis" }
});
